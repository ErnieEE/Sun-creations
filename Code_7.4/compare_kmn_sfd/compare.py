    ...
    with open(out, 'w') as f:
        ...
        for uec in pw_name:
            ...
            if kmn_uec.get(name) == None:
                ...
                print('{:8s}{:20s} not in kmn found --{}'.format(uec, name, kmn_name.get(uec)))
                f.write('{:8s}{:20s} not in kmn found --{}\n'.format(uec, name, kmn_name.get(uec)))
        print('\nCompare unicodes\n')
        f.write('\nCompare unicodes\n')
        for name in pw_uec:
            uec = pw_uec[name]
            ...
            if kmn_name.get(uec) == None:
                ...
                print('{:8s}{:20s} not in kmn found --{}'.format(uec, name, kmn_uec.get(name)))
                f.write('{:8s}{:20s} not in kmn found --{}\n'.format(uec, name, kmn_uec.get(name)))
        ...
        for name in kmn_uec:
            ...
            if pw_name.get(uec) == None:
                print('{:8s}{:20s} not in kmn found --{}'.format(uec, name, pw_uec.get(name)))
                f.write('{:8s}{:20s} not in kmn found --{}\n'.format(uec, name, pw_uec.get(name)))
        f.close()
