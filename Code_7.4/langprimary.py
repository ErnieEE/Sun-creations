...
with open(pwfile, 'r', encoding="utf-8") as f:
    ...
    for row in reader:
        ...
        uec = row[1].strip().strip('"').strip('"').lower()
        pwname = pwname+':'+name
...
for row in reader:
    ...
    # data in lang file
    lnRow = []
    sym = row[ixSym].strip().strip('"').strip("'")
    name = row[ixEN].strip().strip('"').strip("'")
    uni = row[ixUni].strip().strip('"').strip("'").lower()
    lang = row[ixLang].strip().strip('"').strip("'")
    lnRow.append(sym)
    lnRow.append(name)
    lnRow.append(uni)
    lnRow.append(lang)
