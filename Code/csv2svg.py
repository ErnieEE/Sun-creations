...
def makeSVG(fontName, uniName, langName, section):
    ...
    if uniName == 'e37e':
        svgFile = "e37E+period.svg"
        svgCopy(svgFile)
        return
    elif uniName == 'e390':
        svgFile = "e390+possessive.svg"
        svgCopy(svgFile)
        return
    elif uniName == 'ed11':
        svgFile = "ed11+pn.svg"
        ...
        svgFile = "svg\\"+section+"_"+uniName+"+"+langName+".svg"
        
    exists = os.path.isfile(svgFile)
    print('exists',exists)
    if not exists:
        ln = langName.replace(" ","_")
        cmd = "magick convert" + " -font "+fontName+" -pointsize 72 caption:"+ln+" tmp.pnm"
        print(cmd)
        status = subprocess.call(cmd, shell=True)
        if status == 0:
        ...
        if status != 0:    
            print('Error processing ',uniName, langName)
            sys.exit()
    else:
        print('duplicate file',svgFile,'\nMust delete existing files first')
        sys.exit()

def read_list(fontname, csvFile, cg, namelist=""):
    ixu = int(cg["index_unicode"])
    ixn = int(cg["index_name"])

    print('readlist', csvFile, namelist, ixn, type(ixu),type(ixn), type(cg["index_unicode"]))
    with open(csvFile, encoding='utf8') as csvDataFile:
        ...
        for row in csvReader:
            ...
            if (len(row) < 3) or (len(row[ixu])) != 4: 
                continue
            if namelist:
                if row[ixu] not in namelist:
                    continue
            #print(row[ixn], row[ixu], ncol)   
            if len(row[ixu]) < 4:
                makeSVG(fontname, row[ixn].strip(), row[ixn].strip(), cg['alias'])
            else:
                makeSVG(fontname, row[ixu].strip(), row[ixn].strip(), cg['alias'])
...
if len(sys.argv) > 3: 
    ...
    if len(sys.argv) > 4:
        ...
        read_list(fontname, csvFile, cg, namelist)
    else:
        print('whole thing')
