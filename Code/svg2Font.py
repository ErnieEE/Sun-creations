...
def addFont(font, unicode, section, imagename):  #, mn, mx) :
    ...
    '''
    if unicode == 'e37e':
        file = "svg\\e37e_period.svg"
        glyph = font.createChar(int(unicode, 16))
        glyph.importOutlines(file)  
        return
    elif unicode == 'e390':
        file = "svg\\e390_possesive.svg"
        glyph = font.createChar(int(unicode, 16))
        glyph.importOutlines(file) 
        return
    elif unicode == 'ed11':
        file = "svg\\ed11_pn.svg"
        glyph = font.createChar(int(unicode, 16))
        glyph.importOutlines(file) 
        return
    else:
    '''
    file = "svg\\"+section+"_"+unicode+".svg"
...
def read_list(font, csvFile, namelist=""):
    ...
    ixu = int(llMetrics["index_unicode"])   #  index of unicode
    ixn = int(llMetrics["index_name"])      # index of language name
    #ixEN = 1    # index of english name
    #ixu = 2     # unicode index
        ...
        for row in csvReader:
            ...
            if namelist:
                if unicode not in namelist:
                    continue
            print(row[ixn], row[ixu], row[ixEN], ncol) 
            unicode = row[ixu].lower()
            name = row[ixn].strip() 
...
if len(sys.argv) > 4: 
    ...
    font.save(backFont)
    installFont(fnt[0]+".ttf")
    installFont(fnt[0]+".woff")
...
print('done saved font',backFont)
